...
void main()
{
    ...
    texel *= vec2(uLineWidth);
    float perlinNoise1 = texture2D(tPerlin, vUv * vec2(resolution.x / resolution.y, 1.0) * uPerlinFrequency).r - 0.5;
    float perlinNoise2 = texture2D(tPerlin, vUv * vec2(resolution.x / resolution.y, 1.0) * uPerlinFrequency * 3.456).r - 0.5;
    texel += vec2(perlinNoise1 + perlinNoise2) * uPerlinStrength;
    ...
    // gl_FragColor = vec4(vec3(perlinNoise2), 1.0);
}
